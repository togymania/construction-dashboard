...
import {
  ...
  Trash2, ShieldAlert, Bot, TrendingUp, BarChart3, Sparkles,
} from "lucide-react";
import {
...
export default function SubcontractorDetailPage() {
  ...
  // Prefer the backend's subcontractor-wide total (ledger + SubcontractorPayment)
  // because it captures ledger entries that aren't yet attached to a specific
  // contract. Falls back to per-contract sum when sub.total_paid is missing.
  const contractsPaidSum = contracts ? contracts.reduce((s, c) => s + parseFloat(c.paid_amount), 0) : 0;
  const totalPaid = sub?.total_paid !== undefined
    ? parseFloat(sub.total_paid)
    : contractsPaidSum;
  const totalPending = contracts ? contracts.reduce((s, c) => s + parseFloat(c.pending_amount), 0) : 0;

  return (
    <div className="space-y-6">
      <Link href={`/projects/${projectId}/subcontractors`} className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground">
      ...
      <Tabs defaultValue="profile">
        <TabsList>
          <TabsTrigger value="profile"><Sparkles className="h-3.5 w-3.5 mr-1" />Profil</TabsTrigger>
          ...
          <TabsTrigger value="payments"><Wallet className="h-3.5 w-3.5 mr-1" />{t("expenses.payments")}</TabsTrigger>
        ...
        {/* Profile / Firma Kartviziti */}
        <TabsContent value="profile" className="space-y-4">
          <SubcontractorProfileCard subcontractorId={subId} />
        </TabsContent>
                      ...
                      return (
                        <TableRow key={c.id} className="cursor-pointer hover:bg-muted/40">
                          <TableCell className="font-mono text-xs">
                            <Link href={`/projects/${projectId}/subcontractors/${subId}/contracts/${c.id}`} className="hover:underline">{c.contract_number ?? `#${c.id}`}</Link>
                          </TableCell>
            ...
            <CardContent>
              {cfChartData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={cfChartData} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
                    <defs>
                      <linearGradient id="cfPaid" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#10b981" stopOpacity={0.55} />
                        <stop offset="95%" stopColor="#10b981" stopOpacity={0.05} />
                      </linearGradient>
                      <linearGradient id="cfApproved" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#3b82f6" stopOpacity={0.5} />
                        <stop offset="95%" stopColor="#3b82f6" stopOpacity={0.05} />
                      </linearGradient>
                      <linearGradient id="cfPending" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Area type="monotone" dataKey="Paid" stackId="1" stroke="#10b981" strokeWidth={2} fill="url(#cfPaid)" />
                    <Area type="monotone" dataKey="Approved" stackId="1" stroke="#3b82f6" strokeWidth={2} fill="url(#cfApproved)" />
                    <Area type="monotone" dataKey="Pending" stackId="1" stroke="#f59e0b" strokeWidth={2} fill="url(#cfPending)" />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </CardContent>
